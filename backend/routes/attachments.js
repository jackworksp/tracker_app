...
} = require("../utils/attachmentFilters");
...
function buildAttachmentsFilterClause({
  ...
  return `
        WHERE ($${subjectParam}::INTEGER IS NULL OR subject_id = $${subjectParam})
          ...
          AND ${buildPlatformFilterClause({
            platformParam,
            urlExpression: "url",
            platformExpression: "metadata->>'platform'",
          })}
          AND ${buildFileTypeFilterClause({
            fileTypeParam,
            typeExpression: "type",
            urlExpression: "url",
            platformExpression: "metadata->>'platform'",
            attachmentTypeExpression: "metadata->>'attachment_type'",
          })}
    `;
}
...
router.get("/", async (req, res) => {
  try {
    ...
    const platformFilter = normalizePlatform(req.query.platform);
    const fileTypeFilter = req.query.file_type
      ? req.query.file_type.toLowerCase()
      : null;
...
router.post("/", async (req, res) => {
  try {
    ...
    const detectedPlatform =
      normalizePlatform(platform) || detectAttachmentPlatform(url);
  ...
  try {
    ...
    if (idParts[0] === "attachment") {
      ...
      return res.status(400).json({
        error:
          "Cannot delete attachments from tasks or sessions. Please edit the source.",
      });
    }
  } catch (err) {
  ...
  try {
    ...
    if (id.startsWith("attachment-")) {
      ...
      return res
        ...
        .json({
          error:
            "Note links cannot be moved to folders directly. Move the parent task or session instead.",
        });
    } else {
...
router.put("/:id/subject", async (req, res) => {
  try {
    ...
    if (subject_id !== null && !Number.isInteger(normalizedSubjectId)) {
      return res
        .status(400)
        .json({ error: "subject_id must be an integer or null" });
    }
...
router.post("/bulk-move", async (req, res) => {
  try {
    ...
    if (!Array.isArray(attachment_ids) || attachment_ids.length === 0) {
      return res
        .status(400)
        .json({ error: "attachment_ids must be a non-empty array" });
    }
    ...
    for (const id of attachment_ids) {
      try {
        ...
        if (id.startsWith("attachment-")) {
          ...
          id.startsWith("note-task-") ||
          id.startsWith("note-session-")
        ) {
          failedCount++;
