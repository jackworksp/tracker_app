...
const ReminderPicker = ({ task, onSetReminder, onCancel }) => {
  const [reminderTime, setReminderTime] = useState(
    task.reminder_time ? new Date(task.reminder_time).toISOString().slice(0, 16) : ''
  );
  const [alertType, setAlertType] = useState(task.alert_type || 'basic');
